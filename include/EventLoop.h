...
private:
    ...
    static const int Epoll_timeout=1;                                               //epoll超时时间1秒(单位为毫秒)，如果设置为0，理论上很占cpu，因为内核会一直调用epoll，但还未测试

    int epollfd;
    epoll_event events[PerEpollMaxEvent];                                           //epoll在使用时要配一个epoll_event数组

    std::unique_ptr<Chanel> chanelpool[PerEpollMaxEvent];                           //事件池 //llf 中括号的优先级比较高，因此是个数组，元素类型为指针
    //为什么要设置这个数组，原因是使得在监听时可以直接根据fd值来访问对应Chanel,相当于起一个映射的关系；04/06——还有资源管理的目的


   // std::shared_ptr<HttpData> httppool[PerEpollMaxEvent];
