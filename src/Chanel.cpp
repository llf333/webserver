...
void Chanel::CallRevents()
{
    ...
    if(revents & EPOLLERR)
        ...
        //4/7日，在修复长连接出错时调换了顺序，原顺序是 err-in-out-rdhub,会导致一直读数据
        //                               修改为  err-out-rdhub-in
        //按优先级排序，rdhub放在后面会导致一直无法正常断开连接，因为一次只处理一个事件----好像是错的 4/9

        //4-9，写的时候最好还是考虑好优先级，这里在测试之后发现并不影响。
        CallDiscfunc();
...
void Chanel::CallRdfunc()
{
    ...
    else Getlogger()->error("Chanel fd{} don't has read_handle",fd);
}

void Chanel::CallWrfunc()
{
    if(write_hande) write_hande();
    else Getlogger()->error("Chanel fd{} don't has write_handle",fd);
}
void Chanel::CallErfunc()
{
    if(error_handle) error_handle();
    else Getlogger()->error("Chanel fd{} don't has error_handle",fd);
}

void Chanel::CallDiscfunc()
{
    if(disconn_handle) disconn_handle();
    else Getlogger()->error("Chanel fd{} don't has disconn_handle",fd);
}
